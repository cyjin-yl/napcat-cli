...
| Reason | Wake? | Timing |
|--------|-------|--------|
| `AT_ME`, `REPLY_TO_ME`, `DM_ME` | yes | near-immediate (debounce ≤1 s), **cooldown bypassed** |
| `BOT_BANNED`, `BOT_KICKED_FROM_GROUP`, `GROUP_ADMIN_CHANGE`, `MY_MESSAGE_RECALLED` | notify | **immediate**, cooldown bypassed — must read, reply optional |
| `NEW_FRIEND`, `NEW_REQUEST`, `BOT_OFFLINE` | notify | **immediate**, cooldown bypassed — act if needed |
